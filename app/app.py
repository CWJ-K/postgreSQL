...
def show_poll_votes():
    ...
    try:
        for option, votes in zip(options, votes_per_option):
            ...
            print("{option.text} got {votes} votes ({percentage: .2f}% of total)")
...
def randomize_poll_winner():
    ...
    print(f"The random selected wiinner is {winner[0]}.")
...
def menu():
    ...
    while (selection := input(MENU_PROMPT)) != "6":
        try:
            MENU_OPTIONS[selection]()  # Amazing way
        except KeyError:
